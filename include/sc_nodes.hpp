...
class ScProducer : public rclcpp::Node
{
public:
    ScProducer(std::string id, std::size_t length, std::size_t num_iter = 1)
        : Node("sc_producer_" + id)
        , _count(0)
        , _length(length)
        , _num_iter(num_iter)
    {
        ...
        _timer = this->create_wall_timer(
            500ms, 
            std::bind(&ScProducer::timer_callback, this));
    }
...
class ScMatMulGpu : public rclcpp::Node
{
public:
    ScMatMulGpu(std::string id)
        : Node("sc_mat_mul_gpu_" + id)
        , _latency()
        , _exec()
    {
        ...
        _subscription = this->create_subscription<ros_shc::msg::InputData>(
            ...
            std::bind(&ScMatMulGpu::callback, this, std::placeholders::_1));
    }
...
class ScMatMulCpu : public rclcpp::Node
{
public:
    ScMatMulCpu(std::string id)
        : Node("sc_mat_mul_cpu_" + id)
        , _latency()
        , _exec()
    {
        _publisher = this->create_publisher<ros_shc::msg::OutputData>(
        ...
        _subscription = this->create_subscription<ros_shc::msg::InputData>(
            "input" + id, rclcpp::QoS(1).reliable(), 
            std::bind(&ScMatMulCpu::callback, this, std::placeholders::_1));
    }
...
class ScChecker : public rclcpp::Node
{
public:
    ScChecker(std::string id)
        : Node("sc_checker_" + id)
        , _cpu_latency()
        , _gpu_latency()
        , _cpu_frame_id(-1)
        ...
        , _gpu_mat()
    {
        _cpu_subscription = this->create_subscription<ros_shc::msg::OutputData>(
            ...
            std::bind(&ScChecker::cpu_callback, this, std::placeholders::_1));
        _gpu_subscription = this->create_subscription<ros_shc::msg::OutputData>(
            "gpu_output" + id, rclcpp::QoS(1).reliable(), 
            std::bind(&ScChecker::gpu_callback, this, std::placeholders::_1));
    }
